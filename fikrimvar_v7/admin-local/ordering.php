...
<div class="page-head"><div><p class="eyebrow">YAYIN KONTROLÜ</p><h1>Ne görünsün, nerede dursun?</h1><p>Projeyi silmeden ana sayfadan veya arşivden kaldırabilirsin. Satırları sürükleyerek sırala.</p></div></div>
<form method="post"><?= csrf_field() ?><div class="sortable" data-sortable><?php foreach($projects as $i=>$p): ?><article class="sortable-item" draggable="true"><span class="drag-handle">⠿</span><div><strong><?= e($p['title']) ?></strong><small><?= e($p['category_title'] ?? '') ?> · <?= e($p['story_status'] ?? 'hikâye yok') ?></small><input type="hidden" name="projects[<?= (int)$p['id'] ?>][order]" value="<?= $i+1 ?>" data-order-input></div><div class="check-row"><label class="check"><input type="checkbox" name="projects[<?= (int)$p['id'] ?>][home]" <?= $p['show_on_home']?'checked':'' ?>> Ana sayfa</label><label class="check"><input type="checkbox" name="projects[<?= (int)$p['id'] ?>][archive]" <?= $p['show_in_archive']?'checked':'' ?>> Arşiv</label><select name="projects[<?= (int)$p['id'] ?>][section]"><option value="none" <?= $p['home_section']==='none'?'selected':'' ?>>Ana sayfada alan yok</option><option value="focus" <?= $p['home_section']==='focus'?'selected':'' ?>>Büyük hikâye</option><option value="trace" <?= $p['home_section']==='trace'?'selected':'' ?>>Masanın diğer tarafı</option></select></div></article><?php endforeach; ?></div><div class="form-actions"><button class="accent" type="submit">Sırayı kaydet</button></div></form><?php admin_foot(); ?>
