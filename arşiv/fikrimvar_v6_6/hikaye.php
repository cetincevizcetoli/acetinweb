...
require __DIR__ . '/includes/atelier_widget.php';
require __DIR__ . '/includes/blocks.php';

$slug = (string) filter_input(INPUT_GET, 'slug', FILTER_UNSAFE_RAW);
$story = load_story($slug);
$adminPreview = !empty($_SESSION['fikrimvar_admin']['ok']) && isset($_GET['preview']);
if ($story && workshop_is_active($story) && !story_is_published($story)) {
    header('Location: atolye.php?slug=' . rawurlencode((string) ($story['slug'] ?? $slug)));
    exit;
}
if ($story && !story_is_published($story) && !$adminPreview) {
    if (workshop_exists($story)) {
        header('Location: atolye.php?slug=' . rawurlencode((string) ($story['slug'] ?? $slug)));
        exit;
    }
    $story = null;
...
$siteData = load_site();
$site = $siteData['site'] ?? [];
$atelierWidgetConfig = $siteData['atelier_widget'] ?? [];
$allStoriesForWidget = load_stories();
$atelierContext = pinned_atelier_context($siteData, $allStoriesForWidget);
$widgetAtelier = $atelierContext['story'];
$widgetLatest = $atelierContext['latest'];
...
<header class="inner-header">
    <div class="shell inner-header-row">
        ...
        <nav aria-label="Hikâye menüsü"><a href="hikayeler.php">Bütün hikâyeler</a><button type="button" data-atelier-widget-open>Atölye</button><a href="index.php">Ana sayfa</a></nav>
    </div>
</header>
    ...
    <section class="story-signature">
        <div class="shell story-signature-grid" data-reveal>
            ...
            <nav aria-label="Hikâye sonu">
                ...
                <button type="button" data-atelier-widget-open>Canlı atölye <?= icon('arrow') ?></button>
...
<?php render_atelier_widget($widgetAtelier, $widgetLatest, is_array($atelierWidgetConfig) ? $atelierWidgetConfig : []); ?>
