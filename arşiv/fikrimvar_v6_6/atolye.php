...
if ($story && !workshop_exists($story) && story_is_published($story)) {
    header('Location: hikaye.php?slug=' . rawurlencode((string) ($story['slug'] ?? $slug)));
    exit;
}
if (!$story || !workshop_exists($story)) {
    $story = null;
    http_response_code(404);
}
$workshopState = $story ? workshop_status($story) : 'none';
$workshopLabel = match ($workshopState) {
    'open' => 'CANLI ATÖLYE',
    'paused' => 'ATÖLYE BEKLEMEDE',
    'closed' => 'ATÖLYE ARŞİVİ',
    default => 'ATÖLYE',
};
...
<?php else: ?>
    <section class="atelier-hero atelier-hero--v65">
        <div class="shell atelier-hero-grid">
            <div data-reveal>
                <p class="eyebrow"><?= e($workshopLabel) ?> · <?= e(status_label($story)) ?></p>
                <h1><?= e($story['title'] ?? '') ?></h1>
            </div>
            <div data-reveal>
    ...
    <?php if ($story && $workshopState === 'closed'): ?>
    <section class="atelier-closed-note">
        <div class="shell">
            <p class="eyebrow">ATÖLYE KAPANDI</p>
            <h2><?= e((string) ($story['workshop']['closing_note'] ?? 'Bu çalışma Atölyede tamamlandı.')) ?></h2>
            <?php if (story_is_published($story)): ?>
                <a class="button button-light" href="hikaye.php?slug=<?= e(rawurlencode((string) $story['slug'])) ?>">Düzenlenmiş hikâyeyi oku <?= icon('arrow') ?></a>
            <?php else: ?>
                <p>Ham kayıtlar korunuyor; düzenlenmiş hikâye henüz yayımlanmadı.</p>
            <?php endif; ?>
        </div>
    </section>
    <?php endif; ?>
