...
if (!$valid) {
    header('Location: index.php?note=error#notlar');
    exit;
}

$path = DATA_DIR . '/notes-pending.json';
$pending = load_json_path($path);
$pending[] = [
    'name' => $name,
    'message' => $message,
...
$encoded = json_encode($pending, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
$ok = is_string($encoded) && file_put_contents($path, $encoded, LOCK_EX) !== false;
if ($ok) {
    $_SESSION['last_note_at'] = time();
