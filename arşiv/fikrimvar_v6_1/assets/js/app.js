(() => {
  ...
  if (!reducedMotion) {
    ...
    const scheduleOrganicPulse = () => {
      ...
      const delay = 1200 + Math.random() * 2900;
      organicPulseTimer = window.setTimeout(() => {
        const layer = crackLayers[Math.floor(Math.random() * crackLayers.length)];
        const duration = Math.round(900 + Math.random() * 1200);
        layer.style.setProperty('--pulse-duration', `${duration}ms`);
        layer.style.setProperty('--pulse-opacity', (0.58 + Math.random() * 0.24).toFixed(2));
        layer.style.setProperty('--pulse-brightness', (1.95 + Math.random() * 0.55).toFixed(2));
