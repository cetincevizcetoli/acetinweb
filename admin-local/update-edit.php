...
if(is_post()){
  ...
  try{
    ...
    $sortOrder=trim((string)($_POST['sort_order'] ?? ''))!=='' ? (float)$_POST['sort_order'] : admin_next_update_sort_order((int)$project['id']);
    ...
    $st->execute([$slug,$workDate,$display,$title,trim(old('summary')),trim(old('tried')),trim(old('failed')),trim(old('decision')),trim(old('next_step')),trim(old('phase')) ?: 'Genel',checkbox('is_milestone'),$status,(string)($_POST['visibility'] ?? 'public'),checkbox('show_in_recent'),$sortOrder,$status,$id]);
