...
$stats=[
 ...
 'Sitede görünen'=>(int)db()->query("SELECT COUNT(*) FROM stories s JOIN projects p ON p.id=s.project_id WHERE s.deleted_at IS NULL AND s.status='published' AND s.visibility='public' AND p.deleted_at IS NULL AND p.visibility='public' AND p.show_in_archive=1")->fetchColumn(),
 'Hikâyesi yayımlanmış'=>(int)db()->query("SELECT COUNT(*) FROM stories WHERE deleted_at IS NULL AND status='published'")->fetchColumn(),
];
admin_head('Projeler'); ?>
...
<div class="tabs"><?php foreach(['all'=>'Tümü','workshop'=>'Atölyeler','draft'=>'Taslaklar','published'=>'Sitede görünenler','story-published'=>'Hikâyesi yayımlanmış','trash'=>'Çöp kutusu'] as $k=>$v): ?><a class="<?= $filter===$k?'active':'' ?>" href="?filter=<?= e($k) ?>"><?= e($v) ?></a><?php endforeach; ?></div>
<p class="form-hint">Sitede görünenler: proje herkese açık, Hikâyeler sayfasında göster açık ve hikâye yayımlanmış/herkese açık olan kayıtlardır.</p>
